...
**Status:** Architecture frozen. Command set verified.  
**Changelog:** v0.2.2 → v0.2.3: Command set verified against Arroyo Computer Interfacing Manual (Rev 2021-01). Channel selection via `TEC:CHAN <n>` documented. All command tables updated: `TEC:OUTput` (not `TEC:OUT`), `TEC:COND?` (not `TEC:ALARM?`), `TEC:LIMit:ITE`/`TEC:LIMit:V` (not `TEC:LIM:*`), `TEC:R?` (not `TEC:SEN?`), `TEC:FAN?`. Condition register bitmask fully decoded in §4.4. Tier 3 commands updated to verified names. §8.4 sensitive prefixes updated. §8.5 response format uncertainty resolved (bare numeric confirmed). Open questions #2 and #6 resolved.
...
> **Verified command set.** All Arroyo command strings shown in this section have been verified against the Arroyo Computer Interfacing Manual (Rev 2021-01). The 7154 MultiSource is a multi-channel controller: commands are preceded by `TEC:CHAN <n>` to select the active channel. Commands do not take a channel argument directly. Responses are bare numeric values (no unit suffixes). Commands are case-insensitive and terminated by CR, LF, or both. Set commands produce no response; queries (ending in `?`) return one line.
...
| TEC voltage | `TEC:V?` | V | |
| Output state | `TEC:OUTput?` | 0/1 | Returns `0` (off) or `1` (on) |
| Alarm/condition | `TEC:COND?` | integer bitmask | Decoded to summary text; see §4.4 |
...
| Parameter | Arroyo query | Unit | Notes |
|---|---|---|---|
| Current limit (active) | `TEC:LIMit:ITE?` | A | |
| Voltage limit (active) | `TEC:LIMit:V?` | V | |
| Condition register (raw) | `TEC:COND?` | integer | Same query as Tier 1a; raw value for diagnostics |
| Sensor resistance | `TEC:R?` | Ω (ohms) | Raw thermistor/RTD resistance |
| Fan voltage | `TEC:FAN?` | V | Fan on if > 0 |
| Control mode | `TEC:MODE?` | string | Returns `T`, `R`, or `ITE` |
...
| Current limit | `TEC:LIMit:ITE <value>` | ≤ hardware max × safety factor |
| Voltage limit | `TEC:LIMit:V <value>` | ≤ hardware max × safety factor |
| Output enable/disable | `TEC:OUTput <0\|1>` | Allowed only when setpoint is within window |
...
| Parameter | Verified command |
|---|---|
| PID coefficients | `TEC:PID <P>, <I>, <D>` / `TEC:PID?` |
| Sensor type / constants | `TEC:SENsor` / `TEC:CONST` |
| Control mode (write) | `TEC:MODE:T`, `TEC:MODE:R`, `TEC:MODE:ITE` |
| AutoTune | `TEC:AUTOTUNE` / `TEC:AUTOTUNE?` |
| Fan voltage | `TEC:FAN <voltage>` |
| Gain preset | `TEC:GAIN` / `TEC:GAIN?` |
| Heat/cool mode | `TEC:HEATCOOL` / `TEC:HEATCOOL?` |
| Mount preset | `TEC:MOUNT` / `TEC:MOUNT?` |

Network settings are device-level (not `TEC:`-prefixed) and specific to the 7000 Series chassis. Shutdown behaviour is controlled via `TEC:ENABle:OUTOFF`.
...
The gateway decodes the `TEC:COND?` condition register bitmask into human-readable summary text for Tier 1a display. The bit definitions (from the Arroyo Computer Interfacing Manual) are:

| Bit | Value | Meaning |
|---|---|---|
| 0 | 1 | Current limit |
| 1 | 2 | Voltage limit |
| 2 | 4 | Resistance limit |
| 3 | 8 | High temperature limit |
| 4 | 16 | Low temperature limit |
| 5 | 32 | Shorted sensor |
| 6 | 64 | Open sensor |
| 7 | 128 | Open TEC module |
| 9 | 512 | Out of tolerance (status, not alarm) |
| 10 | 1024 | Output on (status, not alarm) |
| 12 | 4096 | Thermal runaway |

For the Tier 1a alarm summary, bits 9 (out of tolerance) and 10 (output on) are masked out as they are status indicators, not fault conditions. The summary shows "NONE" if no alarm bits are set, or a comma-separated list of active alarm names otherwise.
...
**Phase 2** must operate at Level B minimum before bounded setpoint writes are enabled. (Implementation note: the initial Phase 2 prototype ships with Level A as agreed; Level B integration is required before production deployment.)
...
3. Commands matching sensitive prefixes (`TEC:AUTOTUNE`, `TEC:MODE:*`, `TEC:MOUNT`, `TEC:SENsor`, `TEC:CONST`, `TEC:ENABle:OUTOFF`) require a second confirmation step.
...
|---|---|---|---|
| Setpoint | `TEC:SET:T?` | Exact match or ≤ 0.01 °C | Command-level quantity; should match precisely |
| Current limit | `TEC:LIMit:ITE?` | Exact match or ≤ 0.01 A | |
| Voltage limit | `TEC:LIMit:V?` | Exact match or ≤ 0.01 V | |
| Output state | `TEC:OUTput?` | Exact match (0 or 1) | Binary; no tolerance |
...
The tolerance is defined as 0.01, corresponding to 1 unit in the least significant displayed digit. The instrument returns bare numeric values (no unit suffixes), confirmed from the Arroyo Computer Interfacing Manual. All commands must be preceded by `TEC:CHAN <n>` to select the target channel.
...
auth:
  ...
  max_failed_logins: 5
  # Phase 2: config-based role bypass (no login required).
  # Remove or set to null when real auth is implemented in Phase 3.
  default_user: "technician"
  default_role: "technician"         # "viewer" | "technician" | "admin"
...
| T4 | Lock request while DAQ is active (Level B) | Request denied; UI shows `CONTESTED`; gateway remains read-only | 2 |
...
2. ~~**Exact Arroyo command set.**~~ **Resolved.** Command set verified against the Arroyo Computer Interfacing Manual (Rev 2021-01). Channel selection via `TEC:CHAN <n>`. Responses are bare numeric values. Condition register `TEC:COND?` replaces provisional `TEC:ALARM?`. Output command is `TEC:OUTput`, not `TEC:OUT`. All commands updated in §4 and driver code.
...
4. **Sensor types in use.** The Tier 1b readback parsing depends on whether channels use thermistors, RTDs, or other sensor types. `TEC:R?` returns the raw sensor resistance; `TEC:SENsor?` returns the sensor type. This affects unit display and calibration visibility.
...
6. ~~**Alarm bit definitions.**~~ **Resolved.** Condition register `TEC:COND?` bitmask fully decoded in §4.4 from the Arroyo Computer Interfacing Manual.
